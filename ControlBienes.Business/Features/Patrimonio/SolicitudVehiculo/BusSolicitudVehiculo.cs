...
{
	public class BusSolicitudVehiculo : IBusSolicitudVehiculo
	{
		...
		private readonly ILogger<BusSolicitudVehiculo> _logger;
		private readonly IValidator<EntSolicitudVehiculoRequest> _validator;
		...
		public BusSolicitudVehiculo(
			...
			IMapper mapper,
			ILogger<BusSolicitudVehiculo> logger, 
			IValidator<EntSolicitudVehiculoRequest> validator)
		{
		...
		private void BValidarRequest(EntSolicitudVehiculoRequest request)
		{
		...
		private async Task BValidarRequestBD(EntSolicitudVehiculoRequest request)
		{
		...
		public async Task<EntityResponse<int>> BActualizarSolicitudAsync(long idSolicitud, EntSolicitudVehiculoRequest request)
		{
		...
		public async Task<EntityResponse<int>> BCrearSolicitudAsync(EntSolicitudVehiculoRequest request)
		{
		...
		public async Task<EntityResponse<EntSolicitudVehiculoResponse>> BObtenerSolicitudAsync(long idSolicitud)
		{
			var nombreMetodo = nameof(BObtenerSolicitudAsync);
			var resultado = new EntityResponse<EntSolicitudVehiculoResponse>();
			...
			try
			{
				var entidad = await _repositorio.DObtenerRegistroAsync(e => 
					e.iIdSolicitud == idSolicitud && e.iIdTipoBien == (long)EnumTipoBien.MaquinariaVehiculo)
					?? throw new BusRecursoNoEncontradoException(EntMensajeConstant.NoEncontrado);

				Expression<Func<EntSolicitud, bool>> filtro = e => e.iIdSolicitud == idSolicitud;
				var resultDto = await _repositorio.DObtenerProyeccionElementoAsync<EntSolicitudVehiculoResponse>(predicado: filtro);
				resultado.Success(resultDto, _code);
			}
			catch (Exception ex)
			{
				resultado = BusExceptionHandler.Handle<EntSolicitudVehiculoResponse>(
					_logger,
					ex,
					_codeError,
					nombreMetodo,
		...
		public async Task<EntityResponse<IEnumerable<EntSolicitudVehiculoResponse>>> BObtenerSolicitudesPorFiltroAsync(
			long? periodo,
			...
			string unidadAdministrativa)
		{
			...
			var resultado = new EntityResponse<IEnumerable<EntSolicitudVehiculoResponse>>();
			...
			try
			{
				...
				var resultListDto = await _repositorio.DObtenerProyeccionListaAsync<EntSolicitudVehiculoResponse>(filtro);
				resultado.Success(resultListDto, _code);
			}
			catch (Exception ex)
			{
				resultado = BusExceptionHandler.Handle<IEnumerable<EntSolicitudVehiculoResponse>>(
					_logger,
					ex,
					_codeError,
					nombreMetodo,
