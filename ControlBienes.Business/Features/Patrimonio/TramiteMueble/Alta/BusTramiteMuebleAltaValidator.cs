...
{
	public class BusTramiteMuebleAltaValidator : AbstractValidator<EntDetalleAltaMuebleRequest>
	{
		public BusTramiteMuebleAltaValidator()
		{
			RuleFor(request => request.IdSolicitud)
			   .Cascade(CascadeMode.Stop)
			   .Must(id => id > 0).WithMessage("Se requiere el Folio de la Solicitud");

			RuleFor(request => request.NumeroBienes)
			   .Cascade(CascadeMode.Stop)
			   .Must(id => id > 0).WithMessage("Se requiere el Número de Bienes");
			...
			RuleFor(request => request)
			   .Cascade(CascadeMode.Stop)
			   .NotEmpty().WithMessage("Se requiere el No Serie")
			   .Must(value => value.NoSeries.Split(",").Length == value.NumeroBienes).WithMessage("El Número de Bienes y la cantidad de No de Serie no coinciden");
			...
			RuleFor(request => request.FolioFactura)
			   .Cascade(CascadeMode.Stop)
			   .NotEmpty().WithMessage("Se requiere el Folio de la Factura");
			...
			RuleFor(request => request.PrecioUnitario)
			   .Cascade(CascadeMode.Stop)
			   .Must(e => e > 0.0m).WithMessage("El Precio Unitario debe ser mayor a 0.00");
			...
			RuleFor(request => request.ObservacionBien)
			   .Cascade(CascadeMode.Stop)
			   .NotEmpty().WithMessage("Se requiere la Observación del Bien");
